...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (unlikely(afl->custom_mutators_count)) { //自定义变异 不用看
  ...
  orig_in = in_buf = queue_testcase_get(afl, afl->queue_cur);//当前测试用例数据
  ...
  if (unlikely(afl->queue_cur->cal_failed)) { //校准测试用例
  ...
  memcpy(out_buf, in_buf, len);
   ...
   /////////////////////////////////////change
  //ACTF("change begin");
  //ACTF("%s,%d",afl->queue_cur->fname,afl->queue_cur->len);
  char *filename2=NULL;
  u8 *fnn1;
  FILE *file1 =NULL;
  FILE *file2 =NULL;
  ...
  u32 zero_count = 0, one_count = 0,one1_count;

  char *filename=strstr(afl->queue_cur->fname, "orig:");
  filename+=5;  // 跳过最后一个 '/'
  const char *filename1 = strrchr(filename, '.');
  size_t length1 = filename1 - filename;
  filename2 = (char *)malloc(length1 + 5);
  strncpy(filename2, filename, length1);
  filename2[length1] = '\0';
  fnn1 = alloc_printf("%s/%s.bin", afl->in_dir2,filename2);
  file1 = fopen(fnn1, "rb");
  free(filename2);
  free(fnn1);
  fnn1=NULL;
  ...
  ones = (int *)malloc((len+50) * sizeof(int));
  ...
  fclose(file1);
  u32 *ones1 = (int *)malloc((len*3+50) * sizeof(int));
  memcpy(ones1, ones, one_count * sizeof(int));
  one1_count=one_count;
  afl->queue_cur->fs1=ones1;
  afl->queue_cur->len1=one1_count;
  //////////////////////////////////////
...
#define FLIP_BIT(_ar, _b)                     \
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (!skip_eff_map[afl->stage_cur_byte]) continue; //这里可以补充跳过
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (!skip_eff_map[afl->stage_cur_byte]) continue; //这里修改第二个
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (!skip_eff_map[afl->stage_cur_byte]) continue; //第三个
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (!skip_eff_map[afl->stage_cur_byte]) continue; //第四个
...
skip_user_extras:
  ...
  for (i = 0; i <= (u32)len; ++i) {
    ...
    if (!skip_eff_map[i % len]) continue; //这里
...
havoc_stage:
  ...
  */
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) { //开始循环havoc
    //ACTF("begin:%d",one1_count);
    memcpy(ones1, ones, one_count * sizeof(int));
    one1_count=one_count;
    //ACTF("begin:%d",one1_count);
    ...
    for (i = 0; i < use_stacking; ++i) { //一次变异是多少次组合
    ...
    retry_havoc_step: {
      ...
      /*
      if(ones1[one1_count-1]>temp_len){
        ACTF("#######################################################");
        r=1/0;
      }
      ACTF("%d,%d",mutation_array[r],one1_count);
      */
      switch (mutation_array[r]) {
        case MUT_FLIPBIT: {
          ...
          u32 off=ones1[rand_below(afl, one1_count-1)];
          //off=content[rand_below(afl, len1)]
          out_buf[off] ^= 1 << bit;
        ...
        case MUT_INTERESTING8: {
          ...
          out_buf[ones1[rand_below(afl, one1_count-1)]] = interesting_8[item];
        ...
        case MUT_INTERESTING16: {
          ...
          if (unlikely(one1_count < 2 )) { break; }  // no retry
          ...
          //    interesting_16[item];
          *(u16 *)(out_buf + ones1[rand_below(afl, one1_count-2)]) =interesting_16[item];
          break;
        ...
        case MUT_INTERESTING16BE: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          *(u16 *)(out_buf + ones1[rand_below(afl, one1_count-2)]) =
              SWAP16(interesting_16[item]);
//ones[rand_below(afl, one_count)]
          break;
        ...
        case MUT_INTERESTING32: {
          ...
          if (unlikely(one1_count < 4 )) { break; }  // no retry
          ...
          *(u32 *)(out_buf + ones1[rand_below(afl, one1_count-4)]) =
              interesting_32[item];
        ...
        case MUT_ARITH8_: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          out_buf[ones1[rand_below(afl, one1_count-1)]] -= item;
          break;
        ...
        case MUT_ARITH16_: {
          ...
          u32 pos = ones1[rand_below(afl, one1_count-2)];
          item = 1 + rand_below(afl, ARITH_MAX);
        ...
        case MUT_RAND8: {
          ...
          u32 pos = ones1[rand_below(afl, one1_count-1)];
        ...
        case MUT_CLONE_COPY: {//1
          ...
          if (likely(temp_len + HAVOC_BLK_XL < MAX_FILE)) {
            ...
            u32 diffd=rand_below(afl, one1_count-1);
            u32 clone_to = ones1[diffd];
            ...
            for (u32 j = one1_count-1; j >= diffd; j--) {
            //ACTF("1:%d,%d,%d,%d,%d",j,diffd,ones1[j],clone_len,len);              
              if(j==0){
                ones1[j+clone_len]=ones1[j]+clone_len; 
                break;
              }
              ones1[j+clone_len]=ones1[j]+clone_len;             
            }  
            for (u32 j = diffd; j < diffd+clone_len; j++) {
              ones1[j]=clone_to+j-diffd;             
            }  
            one1_count+=clone_len;
        ...
        case MUT_OVERWRITE_COPY: {
          ...
          do {
            ...
            copy_from = ones1[rand_below(afl, one1_count-1)];
            copy_to = ones1[rand_below(afl, one1_count-1)];

          } while (unlikely(copy_from == copy_to || copy_from+copy_len>temp_len || copy_to+copy_len>temp_len));//避免溢出
        ...
        case MUT_OVERWRITE_FIXED: {
          ...
          do{
            copy_to=ones1[rand_below(afl, one1_count-1)];
          }while(unlikely(copy_to+copy_len>temp_len));
          u32 strat = rand_below(afl, 2);
        ...
        case MUT_BYTEADD: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          out_buf[ones1[rand_below(afl, one1_count-1)]]++;
          break;
        ...
        case MUT_FLIP8: {
          ...
          strcat(afl->mutation, afl->m_tmp);
#endif
          out_buf[ones1[rand_below(afl, one1_count-1)]] ^= 0xff;
          break;

        }

        case MUT_SWITCH: {//3 没有动
          ...
          u32 diffd1=rand_below(afl, one1_count-1);
          u32 diffd2;
          switch_from = ones1[diffd1];
          do {
            diffd2=rand_below(afl, one1_count-1);
            switch_to = ones1[diffd2];
        ...
        case MUT_DEL: {//4
          ...
          u32 diffd;
          do{
            diffd=rand_below(afl, one1_count-1);  
            del_from=ones1[diffd]; //可能会无线循环
          }while(unlikely(del_from+del_len>temp_len));
#ifdef INTROSPECTION
          ...
          u32 diffd2=0;
          for (u32 j = diffd; j < one1_count; j++) {
            if(ones1[j]<del_from+del_len){
              diffd2++;
            }else{
              break;
            }           
          } 
          for(u32 j=diffd; j < one1_count-diffd2; j++){
            ones1[j]=ones1[j+diffd2]-del_len;
          }
          one1_count-=diffd2;
        ...
        case MUT_DELONE: {//6
          ...
          u32 del_len = 1;
          u32 diffd=rand_below(afl, one1_count-2);
          u32 del_from = ones1[diffd];
          ...
          temp_len -= del_len;
          if(ones1[diffd]+1==ones1[diffd+1]){
            for(u32 j=diffd;j<one1_count-1;j++){
              ones1[j]=ones1[j+1]-1;
            }
            one1_count-=1;
          }else{
        ...
        case MUT_ASCIINUM: {//7.5 

          if (unlikely(temp_len < 4)) { break; }  // no retry
          u32 diffd=rand_below(afl, one1_count);
          u32 off = ones1[diffd], off2 = off, cnt = 0;
        ...
        case MUT_EXTRA_OVERWRITE: {//不用动
          ...
          insert_at=ones1[rand_below(afl, one1_count-1)];
          if (unlikely(extra_len+insert_at > temp_len)) { goto retry_havoc_step; }
#ifdef INTROSPECTION
        ...
        case MUT_AUTO_EXTRA_OVERWRITE: {
          ...
          u32 insert_at = ones1[rand_below(afl, one1_count-2)];
          if (unlikely(extra_len+insert_at > temp_len)) { goto retry_havoc_step; }
#ifdef INTROSPECTION
          snprintf(afl->m_tmp, sizeof(afl->m_tmp),
        ...
        case MUT_AUTO_EXTRA_INSERT: {//有问题
          if (unlikely(!afl->a_extras_cnt)) { goto retry_havoc_step; }
        ...
        case MUT_SPLICE_INSERT: {
          ...
          for(u32 j=diffd;j<one1_count;j++){
            //ACTF("splice:%d,%d,%d,%d",j,ones1[j],clone_len,len);            
            ones1[j]+=clone_len;
          }
    ...
    afl->queue_cur->len1=one1_count;
    if (common_fuzz_stuff(afl, out_buf, temp_len)) { goto abandon_entry; }// 加入路径不偏移 并且产生新的测试用例后 加入新的bin文件
...
retry_splicing:
  ...
  if (afl->use_splicing && splice_cycle++ < SPLICE_CYCLES &&
      afl->ready_for_splicing_count > 1 && afl->queue_cur->len >= 4 && 0) {//直接跳过 取消splice
...
abandon_entry:
  ...
  orig_in = NULL;
  //ACTF("%p,%p,%p,%p",fs,ones,ones1,fs1);
  free(ones);
  free(ones1);
  ones=NULL;
  ones1=NULL;
  afl->queue_cur->fs1=NULL;
  return ret_val;
...
u8 fuzz_one_original0(afl_state_t *afl) {
...
u8 fuzz_one_original2(afl_state_t *afl) {
...
//
  u32 one_count,one1_count;
  u8 *fnn2 = alloc_printf("%s/%06u", afl->in_dir2,afl->queue_cur->id);
  FILE *file2 = fopen(fnn2, "rb");
  fseek(file2, 0, SEEK_END);
  int file_size = ftell(file2);
  fseek(file2, 0, SEEK_SET);
  u32 *ones = (int *)malloc(file_size+50* sizeof(int));
  size_t bytes_read = fread(ones, 1, file_size, file2);
  fclose(file2); 
  one_count=file_size/sizeof(int);
...
//ones[rand_below(afl, one_count)]
        ...
        case MUT_ASCIINUM: {//7.5 
          ...
          if (old_len == new_len) {
            ...
            for(u32 j=diffd+1;j<one1_count;j++){
              ones1[j]+=(new_len-old_len);
            }            
          }
...
u8 fuzz_one(afl_state_t *afl) {
  ...
  */
  ...
  if (afl->limit_time_sig <= 0) { 
      char *orig=strstr(afl->queue_cur->fname, "orig:");
      if(orig){
        key_val_lv_1 = fuzz_one_original(afl); 
      }
      else{
        fuzz_one_original2(afl);       
      }
